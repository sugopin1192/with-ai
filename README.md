...
[Releases ページ](https://github.com/sugopin1192/with-ai/releases) から最新の `with-ai-vX.X.X.zip` をダウンロード → 解凍。
...
拡張機能のいずれかのAPIキーが必要です（用途に合わせて1つ選択）:
...
2. プロバイダーを選択 → APIキーを入力 → モデルを選択 → 保存
...
pnpm dev            # 開発ビルド（ホットリロード）
```

### Chrome に読み込み（開発時）

`chrome://extensions/` → デベロッパーモード ON → 「パッケージ化されていない拡張機能を読み込む」→ `build/chrome-mv3-prod/` を選択。
...
→ GitHub Actions が自動でビルドして Releases に zip を添付します。
